'''
Fixed train_imagenet.py for ImageNet64 DSNO with wandb configuration fix
'''
import os
import random
...
def train(model, model_ema,
    ...
    # setup wandb with proper config conversion
    if use_wandb and wandb and rank == 0:
        ...
        run = wandb.init(
            ...
            config=wandb_config,  # Use converted dict instead of OmegaConf object
            reinit=True,
...
def run(train_loader,
    ...
    # Skip loading pre-trained weights if init_ckpt is commented out or doesn't exist
    if hasattr(config.training, 'init_ckpt') and config.training.init_ckpt:
        try:
